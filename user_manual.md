...
Because CR-39 scans are often stitched gigapixel images, loading the entire image into the GPU memory at once is typically constrained by hardware limits. This tab allows you to slice, crop, and manipulate sub-regions.
...
* **peak_gaussian_sigma (px)**: Smoothing parameter before finding peaks to reduce high-frequency noise.
* **peak_charac_len (m)**: The characteristic spatial length of the peak region.
* **peak_thre_offset**: Intensity threshold offset for peak validation. Lowering it identifies fainter tracks but increases the rate of false positives.
* **peak_closest_dist (m)**: The minimum physical distance allowed between two distinct tracks. Mitigates over-segmentation of single large pits.
...
* **cir_radii_thre_percent**: Defines the threshold at which an ellipse is treated as a circle (normal incidence).
* **eps_allowed_center_offest (m)**: How far the geometric center of the fitted ellipse can drift from the initially detected peak.
...
* For very large datasets, use **Batch Detect** on the `Slicing` tab. Setting your `ext_rate` (extension rate) > 0 helps mitigate the truncation of pits located at the boundaries of sub-images.
